      ...
      <Container className={style.fincrip_main}>
        <div className={style.fincrip_content}>
          ...
          <p className={style.fincrip_para}>
            Humans want to explore space but face a major challenge as commercial space travel has
            not commenced and will equally be competitive when it starts. The Fincrip Protocol is
            here to address this challenge. It empowers you to navigate a digital space, build
            digital assets, earn $FCP tokens, and mint NFTs for sale on the marketplace. All holders
            of the $FCP token can take part in the decision-making process governing the platform by
            exercising the voting power conferred on them by the token.
          </p>
        </div>
        <Container className={style.fincrip_cards}>
          {finCard.map((item, count) => (
            <div key={count} className={style.finFrame_section}>
